...
class GraphRAG():
	def __init__(self, model, encoder, reasoning_effort):
		self.model = model
		self.encoder = encoder
		self.reasoning_effort = reasoning_effort
		self.db_interviewer = DBInterviewer()
		self.rephraser = Rephraser(self.model, self.encoder)
		self.parser = Parser(self.model, self.encoder, self.db_interviewer)
	...
	def get_data(self, og_question):
		try:
			rephrased_question = self.rephraser.rephrase(og_question, self.reasoning_effort)
		...
		for i, question in enumerate(rephrased_question):
			...
			# If something fails (usually the parsing), use LLM to make up cypher query
				...
				try:
					...
					query = llm_answer(self.model, self.encoder, template, question, self.reasoning_effort)
